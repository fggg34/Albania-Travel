        ...
        return $schema
            ->components([
                ...
                Select::make('highlights')
                    ->relationship('highlights', 'title', fn ($query) => $query->orderBy('highlights.sort_order')->orderBy('highlights.title'))
                    ->multiple()
                    ->searchable()
                    ->preload()
                    ->reorderable()
                    ->helperText('Places to visit in this city (shown as circular images carousel on the city page).')
                    ->columnSpanFull(),
