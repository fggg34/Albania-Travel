        ...
        <div class="max-w-4xl mx-auto">
            ...
            <section class="mb-10">
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        @if($bookings->isEmpty())
                            <p class="text-gray-500">You have no bookings yet. <a href="{{ route('tours.index') }}" class="text-amber-600 hover:text-amber-700 font-medium">Browse tours</a> to plan your next adventure.</p>
                        @else
                            <ul class="divide-y divide-gray-200">
                                @foreach($bookings as $booking)
                                    <li class="py-5 first:pt-0 last:pb-0 flex flex-wrap items-center justify-between gap-3">
                                        <div class="flex-1 min-w-0">
                                            <a href="{{ route('bookings.confirmation', ['token' => $booking->confirmation_token]) }}" class="font-medium text-gray-900 hover:text-amber-600 transition">{{ $booking->tour->title }}</a>
                        ...
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                @foreach($wishlistTours as $tour)
                                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-amber-200 transition">
                                        <a href="{{ route('tours.show', $tour->slug) }}" class="font-medium text-gray-900 hover:text-amber-600 transition">{{ $tour->title }}</a>
