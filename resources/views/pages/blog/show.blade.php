        ...
        <div class="flex flex-wrap flex-col items-left gap-4 text-sm">
            @if($post->category)
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('blog.index', ['category' => $post->category->slug]) }}" class="inline-flex items-center px-3 py-1 rounded-full bg-amber-50 text-amber-700 font-medium hover:bg-amber-100 transition-colors">{{ $post->category->name }}</a>
                </div>
            @endif
