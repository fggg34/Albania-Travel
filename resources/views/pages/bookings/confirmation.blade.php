            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-0">
                ...
                <div class="lg:col-span-3 p-6 sm:p-8">
                    ...
                    <div class="flex flex-wrap gap-3 mt-6 pt-5 border-t border-gray-100">
                        ...
                        @auth
                            <a href="{{ route('dashboard') }}" class="inline-flex items-center justify-center px-5 py-2.5 rounded-lg text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 transition">My bookings</a>
                            @if($booking->user_id === auth()->id() && $booking->status !== 'cancelled')
                                <form action="{{ route('user.bookings.cancel', ['token' => $booking->confirmation_token]) }}" method="POST" class="inline" onsubmit="return confirm('Cancel this booking?');">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center justify-center px-5 py-2.5 rounded-lg text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 transition">Cancel booking</button>
                                </form>
                            @endif
                        @else
                            <a href="{{ route('tours.index') }}" class="inline-flex items-center justify-center px-5 py-2.5 rounded-lg text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 transition">Browse more tours</a>
                        @endauth
                    </div>
                </div>
                ...
                <div class="lg:col-span-2 bg-gray-50 p-6 sm:p-8 border-t lg:border-t-0 lg:border-l border-gray-200">
                    ...
                    <div class="mt-5 pt-4 border-t border-gray-200 flex items-center justify-between">
                        ...
                        <span class="text-lg font-bold text-red-600">{{ (strtoupper($booking->currency ?? '') === 'EUR' ? '€' : ($booking->currency ?? '')) }}{{ number_format($booking->total_amount, 2) }}</span>
                    </div>
