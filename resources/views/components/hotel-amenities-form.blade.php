...
<section class="mb-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        @foreach($amenities as $amenity)
            <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:border-amber-500 hover:bg-amber-50/30 cursor-pointer transition">
                <input type="checkbox"
                       name="amenities[]"
                       value="{{ $amenity->id }}"
                ...
                @if($amenity->icon)
                    <i class="{{ $amenity->icon }} w-5 h-5 text-amber-600 flex-shrink-0" aria-hidden="true"></i>
                @else
