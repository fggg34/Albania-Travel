...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('amenities.create') }}" class="inline-flex items-center px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-md hover:bg-amber-700">Add amenity</a>
    </div>
    ...
    <p class="text-sm text-gray-500 mb-4">You can also manage amenities in the <a href="{{ url('/admin') }}" class="text-amber-600 hover:underline">Filament admin panel</a> under Locations → Amenities.</p>
        ...
        @forelse($amenities as $amenity)
            <li class="flex items-center justify-between p-3 rounded-lg border border-gray-200 bg-white">
                <span class="flex items-center gap-3">
                    @if($amenity->icon)
                        <i class="{{ $amenity->icon }} text-amber-600 w-5" aria-hidden="true"></i>
                    @endif
                    <span class="font-medium text-gray-900">{{ $amenity->name }}</span>
                </span>
                <span class="flex items-center gap-2">
                    <a href="{{ route('amenities.edit', $amenity) }}" class="text-sm text-amber-600 hover:underline">Edit</a>
